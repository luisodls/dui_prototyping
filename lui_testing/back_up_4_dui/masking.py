...
phil_scope = parse(
    ...
    .multiple = True
  {
    ...
    rectangle = None
      .type = ints()
      .help = "An untrusted rectangle (x0, x1, y0, y1, panel_number)"
...
def generate_mask(
    ...
    for index, single_panel in enumerate(detector):
        mask = flex.bool(flex.grid(reversed(single_panel.get_image_size())), True)
        ...
        for region in params.untrusted:
            if region.panel == index or region.panel is None:
                if not any(
        ...
        if params.disable_parallax_correction:
            single_panel = copy.deepcopy(single_panel)
            single_panel.set_px_mm_strategy(SimplePxMmStrategy())

        # Generate high and low resolution masks
        if params.d_min is not None:
            logger.debug(f"Generating high resolution mask:\n d_min = {params.d_min}")
            _apply_resolution_mask(mask, beam, single_panel, 0, params.d_min)
        if params.d_max is not None:
            logger.debug(f"Generating low resolution mask:\n d_max = {params.d_max}")
            d_max = params.d_max
            d_inf = max(d_max + 1, 1e9)
            _apply_resolution_mask(mask, beam, single_panel, d_max, d_inf)
        ...
        try:
            ...
            for drange in params.resolution_range:
                ...
                _apply_resolution_mask(mask, beam, single_panel, d_min, d_max)
        except TypeError:
        ...
        for drange in generate_ice_ring_resolution_ranges(
            beam, single_panel, params.ice_rings
        ):
            d_min = min(drange)
            d_max = max(drange)
            assert d_min < d_max, "d_min must be < d_max"
